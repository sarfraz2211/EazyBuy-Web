...
import { addToCart, changeQuantity } from "@/src/redux/cartSlice";
...
// Loads the selected product and connects its quantity buttons to the Redux cart.
export default function ProductDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const productId = decodeURIComponent(id);
  const dispatch = useAppDispatch();
  const merchantId = useAppSelector(state => state.auth.profile?.merchantId ?? "");
      ...
      <main className="product-detail-page">
        ...
        <section className="product-hero">
          ...
          <div className="detail-copy">
            ...
            <div className="detail-rating"><span>★ 4.6</span><a href="#reviews">238 verified ratings</a></div>
            <div className="detail-price">₹{Number(product.price).toLocaleString("en-IN")} <small>inclusive of all taxes</small></div>
            ...
            <div className="delivery-box"><span>⌖</span><div><strong>Delivery across India</strong><small>Usually dispatched within 1–2 business days</small></div></div>
          </div>
        </section>
        ...
        <section className="add-cart-panel">
          <div><small>Your price</small><strong>₹{Number(product.price).toLocaleString("en-IN")}</strong><span>Free delivery available</span></div>
          {cartItem ? <div className="quantity-control" aria-label="Cart quantity"><button onClick={() => dispatch(changeQuantity({ id: productId, amount: -1 }))} aria-label="Decrease quantity">−</button><span>{cartItem.quantity}</span><button onClick={() => dispatch(changeQuantity({ id: productId, amount: 1 }))} aria-label="Increase quantity">+</button></div> : null}
          <button className="add-cart-button" onClick={() => dispatch(addToCart(product))}>{cartItem ? "Add one more" : "Add to cart"}<span>→</span></button>
        </section>
      </main>
      <Footer />
    </div>
